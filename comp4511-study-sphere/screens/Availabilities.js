...
import { Text, View, ScrollView, StyleSheet, TouchableOpacity, useWindowDimensions, Button } from "react-native";
...
const getAvailabilityText = (color) => {
  ...
  const GroupAvailabilitiesScene = () => {
    ...
    const handleSelect = (title) => {setSelectedButton(title);};
    ...
    const renderAvailability = () => {
      ...
      return timeSlots.map((slot, index) => (
        <View
          key={index}
          style={[buttonstyle2(getButtonColor(slot.availability)), styles.timeSlotButton]}
        >
          <Text style={styles.timeText}>
            {slot.time} - {getAvailabilityText(getButtonColor(slot.availability))}
          </Text>
        </View>
      ));
    };
      ...
      >
        <ScrollView contentContainerStyle={styles.containerInner}>
          ...
          <View style={styles.buttonContainer}> 
            <ToggleButton title="COMP3511" isSelected={selectedButton === 'COMP3511'} onSelect={handleSelect} />
            <ToggleButton title="COMP4511" isSelected={selectedButton === 'COMP4511'} onSelect={handleSelect} />
          </View>
         </View>
         <Text style={styles.title}>{"   "}Select a Date To View Availability</Text>
          <View style={styles.calendarContainer}>
          <View style={styles.calendarInnerContainer}>
          <CalendarPicker
            onDateChange={changeDate2}
            width={300}
            selectedStartDate={selectedDate ? new Date(selectedDate) : new Date()}
          />
          </View>
          <View style={styles.timeSlotsContainer}>
          {selectedDate && (<Text style={styles.dateText}>{dateFormat(selectedDate)}</Text>
            )}
            {renderAvailability()}
          </View>
          </View>   
  ...
  return (
    <TabView
      ...
      renderScene=
      {
        SceneMap({myAvailability: MyAvailabilityScene, groupAvailabilities: GroupAvailabilitiesScene,})
      }
